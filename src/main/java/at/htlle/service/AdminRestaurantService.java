...
public class AdminRestaurantService {
    ...
    public Branch createBranch(Long restaurantId, String branchCode, String name, boolean defaultBranch) {
        Restaurant restaurant = restaurantRepository.findById(restaurantId)
                .orElseThrow(() -> new IllegalArgumentException("Restaurant not found"));
        String normalizedCode = normalizeCode(branchCode);
        branchRepository.findByRestaurantIdAndBranchCode(restaurantId, normalizedCode)
                .ifPresent(existing -> {
                    throw new IllegalArgumentException("Branch code already exists for this restaurant");
                });
        Branch branch = new Branch();
        branch.setRestaurant(restaurant);
        branch.setBranchCode(normalizedCode);
    ...
    public Branch updateBranch(Long branchId, Long restaurantId, String branchCode, String name, boolean defaultBranch) {
        ...
        String normalizedCode = normalizeCode(branchCode);
        branchRepository.findByRestaurantIdAndBranchCode(restaurantId, normalizedCode)
                .filter(existing -> !existing.getId().equals(branchId))
                .ifPresent(existing -> {
                    throw new IllegalArgumentException("Branch code already exists for this restaurant");
                });
        branch.setRestaurant(restaurant);
